...
@Singleton
public class GrpSecuritySecuredAnnotationRule implements SecurityRule {

    /**
     * The order of the rule.
     */
    public static final Integer ORDER = SecuredAnnotationRule.ORDER - 50;
     ...
     */
    GrpSecuritySecuredAnnotationRule(TokenConfiguration tokenConfiguration) {
        this.tokenConfiguration = tokenConfiguration;
    }

    @Override
    public SecurityRuleResult check(HttpRequest request, @Nullable RouteMatch routeMatch, @Nullable Map<String, Object> claims) {
        Optional<List> optionalAnnoValues = request.getAttribute(GrpcSecurityHttpRequest.ANNOTATION_VALUES, List.class);
        if (optionalAnnoValues.isPresent()) {
            List<String> annoValues = optionalAnnoValues.get();
            if (annoValues.contains(SecurityRule.DENY_ALL)) {
                return SecurityRuleResult.REJECTED;
            }
            return compareRoles(annoValues, getRoles(claims));
        }
